...
| File | Purpose |
|------|---------|
| `src/hooks/useElevenLabs.ts` | ElevenLabs WebSocket pattern |
...
**Response:**
```typescript
{ success: boolean; data: RoadmapSchema }
```
...
### ElevenLabs Flow (if applicable)
1. `setup_elevenlabs_socket` → open WebSocket
...
- [ ] Error state shown on failure (ElevenLabs / Bedrock)
...
3. Check error state: disconnect ElevenLabs → UI shows error cleanly
