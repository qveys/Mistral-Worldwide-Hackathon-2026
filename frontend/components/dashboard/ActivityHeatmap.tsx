'use client';

import React from 'react';
import { cn } from '@/lib/utils';
import { Badge } from '@/components/ui/Badge';

interface ActivityHeatmapProps {
  data: number[];
  title?: string;
  className?: string;
}

export function ActivityHeatmap({ data, title = "Neural Activity", className }: ActivityHeatmapProps) {
  return (
    <div className={cn("bg-[#161618] border border-zinc-800/50 rounded-[2rem] p-8 flex flex-col justify-between h-full", className)}>
      <div className="flex justify-between items-center">
        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500">{title}</h3>
        <Badge className="bg-violet-500/10 text-violet-400 border-none text-[9px]">Live</Badge>
      </div>
      <div className="grid grid-cols-14 grid-rows-7 gap-1.5 h-36 w-full mt-4">
        {data.map((level, i) => (
          <div 
            key={i} 
            className={cn(
              "rounded-[2px] w-full h-full transition-colors duration-500",
              level === 0 ? "bg-zinc-800/50" : 
              level === 1 ? "bg-violet-900/40" : 
              level === 2 ? "bg-violet-600/60" : 
              "bg-violet-400"
            )} 
          />
        ))}
      </div>
      <div className="flex items-center justify-between text-[10px] font-mono text-zinc-600 mt-4">
        <span>14 WEEK FLOW</span>
        <span className="text-white font-bold tracking-widest uppercase">{data.filter(l => l > 0).length} Syncs</span>
      </div>
    </div>
  );
}
