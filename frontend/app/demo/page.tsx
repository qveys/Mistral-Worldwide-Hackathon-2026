...
import { Sparkles, Layout, Play, Moon, Sun, BookOpen } from 'lucide-react';
  ...
  return (
    <div className={cn(
      ...
      <div className="relative z-10 p-8 space-y-12 max-w-6xl mx-auto">
        {/* Navbar */}
        <nav className="flex justify-between items-center bg-white/10 dark:bg-black/20 backdrop-blur-md border border-white/20 dark:border-white/5 p-4 rounded-2xl">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setView('flow')}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all",
                view === 'flow' ? "bg-blue-600 text-white shadow-lg shadow-blue-500/30" : "text-slate-500 hover:bg-white/10"
              )}
            >
              <Play size={18} />
              Full Flow
            ...
            <button 
              ...
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all",
                view === 'catalog' ? "bg-blue-600 text-white shadow-lg shadow-blue-500/30" : "text-slate-500 hover:bg-white/10"
              )}
            >
              <BookOpen size={18} />
              Component Library
          ...
          <div className="flex items-center gap-4">
            <button 
              ...
              className="p-3 bg-white/20 dark:bg-black/40 rounded-xl border border-white/20 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:scale-110 transition-all"
            >
