...
- **Proxy** : `app/frontend/proxy.ts` (Next.js 16) pour la détection de locale et les redirections.
...
- Les métadonnées (title, description, openGraph) sont définies par `generateMetadata` dans `app/frontend/pages/[locale]/layout.tsx` à partir des clés `metadata.*`.
- `<html lang={locale}>` est défini dans le layout racine via le header `x-next-intl-locale` envoyé par le proxy.
