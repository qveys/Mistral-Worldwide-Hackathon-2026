...
**Architecture:** Frontend Next.js 15 (App Router) + Backend Express TypeScript déployé sur Lambda. Pipeline : ElevenLabs WebSocket STT → Mistral Large (AWS Bedrock) → Roadmap JSON validée par Zod → UI React.

**Tech Stack:** Next.js 15, TailwindCSS, Framer Motion, Express TypeScript, AWS Bedrock (Mistral Large), ElevenLabs WebSocket SDK, Zod, AWS Lambda/Amplify
...
> Objectif : Speech-to-text via ElevenLabs → roadmap structurée par Mistral → UI fonctionnelle pour la démo 48h.

---

### 🏗️ [EPIC 1] Fondation Partagée

*Bloquant pour tout le reste — à faire en PREMIER, en parallèle sur 3 fichiers distincts.*
...
### 🎤 [EPIC 2] Voice Input — ElevenLabs STT

*Le cœur fonctionnel du produit : parler → texte en temps réel.*

| ID | Titre | Assigné | Priorité |
|----|-------|---------|----------|
| **2-1** | Backend `POST /transcribe` — intégration ElevenLabs WebSocket audio → texte | @alice-444 | 🔴 critical |
| **2-2** | Hook `useElevenLabs` — WebSocket client, CommitStrategy.VAD, states (idle/listening/transcribing) | @Matlavv | 🔴 critical |
...
*Transformer le brain dump texte en roadmap JSON structurée.*
...
*Persistance minimale pour le hackathon + partage par lien.*
...
*L'expérience utilisateur principale : saisie → affichage roadmap.*
...
*Affiner la roadmap conversationnellement.*
...
*Garantir que la démo fonctionne même si un service externe est down.*

| ID | Titre | Assigné | Priorité |
|----|-------|---------|----------|
| **7-1** | `DEMO_MODE` env flag — mock JSON activable, bypass Bedrock + ElevenLabs | @Matlavv | 🔴 critical |
| **7-2** | Fallback input texte — bypass ElevenLabs si WebSocket down, textarea directe | @alice-444 | 🟠 high |
| **7-3** | Error States UI — toast notification, bouton "Réessayer", Error Boundary React | @qveys | 🟠 high |
| **7-4** | ElevenLabs WebSocket — reconnexion avec exponential backoff (max 3 tentatives) | @alice-444 | 🟠 high |
...
*Feature différenciante : tâches prérequises, détection de cycles, UI bloquée/disponible.*
...
*Composants réutilisables + accessibilité + responsive.*
...
*Stabilité Lambda, monitoring, santé API.*
...
*Démarrer avec un template au lieu d'un brain dump vide.*
...
*L'IA analyse le brain dump en direct et pose des questions de clarification.*
...
*XP, streaks, achievements pour complétion de tâches.*
...
*Partage public read-only + visualisation graphe de dépendances.*
...
**@Matlavv** (AI/Hooks/Schema): 1-1, 2-2, 3-2, 3-4, 6-2, 7-1, 8-1, 8-2, 8-4, 8-6, 9-4, 11-2, 13-1, 13-2 → **14** *(+ 14-2 si @qveys surchargé)* → **15**
...
Fermer avec commentaire : *"Réorganisation feature-based — remplacé par le plan d'implémentation [docs/plans/2026-02-28-echomaps-implementation.md]"*
