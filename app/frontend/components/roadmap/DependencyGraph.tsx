        ...
        >
            ...
            <div className="flex flex-col gap-1">
                ...
                <div className="mt-2 flex items-center gap-2">
                    ...
                    <div className="text-[8px] font-bold text-slate-400">Size: {task.estimate}</div>
                </div>
            </div>
...
export function DependencyGraph({ tasks, onStatusChange, className }: DependencyGraphProps) {
    const { nodes: initialNodes, edges: initialEdges } = useMemo(() => {
        ...
        const edges: Edge[] = [];
        tasks.forEach((task) => {
            (task.dependsOn ?? task.dependencies ?? []).forEach((depId: string) => {
                edges.push({
                    id: `e-${depId}-${task.id}`,
                    source: depId,
                    target: task.id,
    ...
    return (
        <div
            className={cn(
                'w-full h-[600px] bg-slate-50/50 dark:bg-black/20 rounded-[2.5rem] border border-white/20 overflow-hidden shadow-inner',
                className,
            )}
        >
            <ReactFlow
