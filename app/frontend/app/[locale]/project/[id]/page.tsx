...
import type { Planning, Task as RoadmapTask, TaskStatus } from '@/lib/types';
import { roadmapToMarkdown } from '@/lib/types';
import { useStructure } from '@/lib/useStructure';
import { cn } from '@/lib/utils';
import { AnimatePresence, motion } from 'framer-motion';
import { Activity, AlertCircle, History, Lock, Network, Sparkles, Zap } from 'lucide-react';
...
export default function ProjectPage() {
    ...
    // Pattern React recommandé : mise à jour conditionnelle pendant le rendu
    // évite le useEffect→setState qui cause des rendus en cascade
    if (roadmap && projectId !== syncedProjectId) {
    ...
    const handleRevision = useCallback<(revision: string) => void>(() => {}, []);
        ...
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 text-slate-700 dark:text-slate-100 transition-colors duration-300">
                <div className="w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/50 rounded-[2.5rem] p-10 shadow-2xl text-center space-y-6">
                    ...
                    <p className="text-slate-600 dark:text-zinc-500 italic">
                        &quot;{localizedError}&quot;
                    </p>
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-100 font-sans selection:bg-blue-200 dark:selection:bg-blue-500/30 transition-colors duration-300">
            ...
            <main className="max-w-7xl mx-auto px-6 py-12 space-y-16">
                <div className="space-y-10">
                    <SectionHeader title={roadmap.title} description={t('sectionDescription')} />
