...
export const DEMO_REVISED_ROADMAP = {
  roadmap: DEMO_ROADMAP.roadmap
    .map((item, index) => ({
      ...item,
      ...(index === 0 && {
        description:
          item.description +
          ' Added biometric authentication (Face ID / fingerprint) as a secondary login method.',
        priority: 1,
      }),
    }))
    .concat([
      {
        ...
        description:
          'Implement offline-first architecture with local SQLite caching. Allow users to browse previously loaded feed content without connectivity and sync when back online.',
        priority: 2,
        dependsOn: ['task-003'],
      },
    ]),
